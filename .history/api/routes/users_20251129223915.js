...
router.get('/setup-roles', async (req, res) => {
    try {
        ...
        // 1. Senin Kullanıcını Bul
        let user = await Users.findOne({ email: "bilalerkoc@gmail.com" }); 
        if (!user) return res.send("HATA: Kullanıcı (bilalerkoc@gmail.com) bulunamadı! Lütfen email adresini kontrol et.");

        // 2. TEMİZLİK: Eski Rolleri, İzinleri ve Bağlantıları SİL
        ...
        // 3. Yeni "Super Admin" Rolü Oluştur
        let role = await Roles.create({
        ...
        // 4. Yetkileri Tanımla (Config dosyandaki isimlerle BİREBİR AYNI olmalı)
        // Burada hem tekil (user_add) hem çoğul (users_add) ekliyorum ki garanti olsun.
        let permissions = [
            "user_view", "user_add", "user_update", "user_delete",
            "users_view", "users_add", "users_update", "users_delete", // Ne olur ne olmaz
        ...
        res.send(`İŞLEM BAŞARILI! <br> 
                  1. Eski ayarlar silindi. <br> 
                  2. 'Super Admin' rolü oluşturuldu (ID: ${role._id}). <br> 
                  3. Tüm yetkiler yüklendi. <br> 
                  4. Kullanıcı (${user.email}) bu role bağlandı. <br><br>
                  Lütfen ÇIKIŞ yapıp tekrar GİRİŞ yapın.`);
