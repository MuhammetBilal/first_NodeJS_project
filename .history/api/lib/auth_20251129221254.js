    ...
    let strategy = new Strategy({
        ...
        try {
            ...
            if (user) {
                ...
                console.log("------------------------------------------------");
                console.log("1. KULLANICI:", user.email);
                console.log("2. BULUNAN HAM YETKİLER (DB):", rolePrivileges.map(x => x.permission));
                let privileges = rolePrivileges.map(rp => privs.privileges.find(x => x.key == rp.permission))
                let eslesmeyenler = rolePrivileges.filter(rp => !privs.privileges.find(x => x.key == rp.permission));
                if(eslesmeyenler.length > 0) {
                    console.log("⚠️ DİKKAT: Config dosyasında bulunamayan yetkiler var:", eslesmeyenler.map(x => x.permission));
                }
                console.log("3. EŞLEŞEN YETKİLER (Config):", privileges.filter(x => x).map(x => x?.key));
                console.log("------------------------------------------------");
                done(null, {
                    id: user._id,
                    roles: privileges,
