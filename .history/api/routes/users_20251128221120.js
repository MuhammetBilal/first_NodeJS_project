...
router.post("/auth", async (req, res) => {
  try {
    console.log("1. İstek geldi, body:", req.body);
    
    let { email, password } = req.body;

    // Validasyon kontrolü
    Users.validateFieldsBeforeAuth(email, password);
    console.log("2. Validasyon başarılı, Veritabanı aranıyor...");

    // BURASI ÇOK ÖNEMLİ: Veritabanı sorgusu
    let user = await Users.findOne({ email });
    console.log("3. Veritabanı sorgusu bitti. Kullanıcı:", user ? "Bulundu" : "Bulunamadı");
    ...
    console.log("4. Şifre doğru, Token üretiliyor...");
    ...
    console.log("5. İşlem tamam, cevap gönderiliyor.");
    res.json(Response.successResponse({ token, user: userData }));

  } catch (err) {
    console.error("HATA YAKALANDI:", err); // Hatayı terminale detaylı yazdırır
    let errorResponse = Response.errorResponse(err);
    res.status(errorResponse.code).json(errorResponse);
  }
});
