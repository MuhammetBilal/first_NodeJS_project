...
router.post("/add", async(req, res, next)=>{
   ...
   try {
        ...
        for( let i=0; i < permission.lenght; i++){
        ...
        if(typeof body.is_active === "boolean") updates.is_active = body.is_active; 
         ...
         if(!body.permission && !Array.isArray(body.permission) && body.permission.lenght > 0) {
               ...
               if(removePermissions.lenght > 0){
                    await RolePrivileges.remove({_id: {$in: removePermissions.map( x => x._id)}}); // veri tabından silme işi
               }

               if(newPermissions.lenght > 0){ // veitabanına ekleme yapılır.
